...
function XMediaTile({ media, crop }: { media: XCardMedia; crop: boolean }) {
  ...
  return image ? (
    // Provider-owned media is loaded only after strict host validation.
...
function XMediaGrid({
  ...
  return (
    <div
      ...
      className={cn(
        "grid overflow-hidden rounded-lg border border-white/10 bg-black/40",
        compact ? "aspect-video w-full" : "min-h-24 flex-1",
...
/** Shared safe TweetDeck-style presentation for free Cards and Column tiles. */
export function XCardContent({
  card,
  variant = "card",
}: {
  card: XCard;
  ...
  return (
    <div
      data-x-card-variant={variant}
      className={cn(
        "nowheel flex min-h-0 flex-col gap-2",
        compact ? "p-3" : "h-full overflow-y-auto pr-0.5",
      )}
    >
      <div className="flex min-w-0 items-center gap-2">
        {avatar ? (
          // X avatars are remote, optional, and validated at the render boundary.
          // eslint-disable-next-line @next/next/no-img-element
          <img
      ...
      />
      <XMediaGrid media={payload.media ?? []} compact={compact} />
