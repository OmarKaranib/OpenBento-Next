...
Status: `running` | `paused` | `error`. `instruction` required on create. Provider-agnostic `SourceProvider`; optional Grok and OpenAI web/news adapters are env-gated and are not imported by domain. First slice sources are web/news only.
...
- **Railway:** **US East / Virginia** — web runtime is hosted; WatchBot worker **implementation is ready**, but worker **deployment is not yet authorized**. Config prepared (`railway.toml` + `railway.worker.toml` with `--provider=x` + [`docs/DEPLOY.md`](./DEPLOY.md)). Do not create/start the worker service or enable live X without owner approval.
