...
- **Decision:** Frames are bordered display regions on the canvas and can be fullscreened. They are not zoom levels and not a semantic hierarchy.
- **Why:** Presentation without changing IA.

## ADR-007 — Shared domain actions for UI / WatchBot / WebMCP

- **Decision:** One catalog in `packages/domain`. Required names: `createWatchBot`, `pauseWatchBot`, `createCard`, `updateCard`. WebMCP tools map 1:1 via `document.modelContext.registerTool({ name, description, inputSchema, execute })`.
