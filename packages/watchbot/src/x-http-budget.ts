...
/** Shared worker-tick budget for actual X HTTP requests across all WatchBots. */
export function xMaxRequestsPerWorkerTick(
  env: NodeJS.ProcessEnv = process.env,
): number {
  return boundedInt(
    env.X_MAX_REQUESTS_PER_WORKER_TICK,
...
export class XHttpBudget {
  ...
  /** Returns false when the global worker-tick budget is exhausted. */
  tryConsume(): boolean {
